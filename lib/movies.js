...
const getMovieData = async (request, response) => {
  ...
  const cacheUrl = `https://api.themoviedb.org/3/search/movie?api_key=${MOVIE_API_KEY}&query=${request.query.city}&include_adult=false`;
  ...
  if (cache[cacheKey]) {
    ...
    if ((Date.now() - cache[cacheKey].timestamp) < 3600000) {
      ...
      cache[cacheKey].response.results.forEach(movie => {
        cacheResponse.push(new Movie(movie.title, movie.overview, movie.vote_average, movie.vote_count, movie.poster_path, movie.popularity, movie.release_date))
      });
